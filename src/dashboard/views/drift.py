...
# Inline replacement for .obsidian-card CSS class (st.html renders in an iframe)
_CARD_STYLE = (
...
def _render_status_badge(status: str) -> None:
    ...
    st.html(
        f"""
        <div style="{_CARD_STYLE} text-align:center; padding:1.5rem;">
...
def _render_summary_cards(summary_stats: Dict[str, int]) -> None:
    ...
    for col, (verdict, count) in zip(cols, summary_stats.items()):
        ...
        col.html(
            f"""
            <div style="{_CARD_STYLE} text-align:center; padding:1rem;">
...
def _render_drift_table(report: DriftReport) -> None:
    ...
    # Build HTML table
    header = """
    <div style="{card} overflow-x:auto; padding:0.5rem;">
    ...
    <tbody>
    """.format(sec=_TEXT_SECONDARY, card=_CARD_STYLE)
...
def _render_actions(report: DriftReport) -> None:
    ...
    if not report.actions:
        st.html(
            f"""
            <div style="{_CARD_STYLE} padding:1rem; border-left:3px solid {_GREEN};">
    ...
    for action in report.actions:
        ...
        st.html(
            f"""
            <div style="{_CARD_STYLE} padding:0.75rem; margin-bottom:0.5rem;
